...
bool Sema::IsOpenMPCapturedVar(VarDecl *VD, Scope *&StopScope) {
  ...
  assert( LangOpts.OpenMP && "Not supporting OpenMP??");
  ...
  DeclRefExpr *CE;
  OpenMPClauseKind Kind = DSAStack->getTopDSA(VD, CE);

  bool isOpenMPPrivate =
      Kind == OMPC_private || Kind == OMPC_firstprivate ||
      Kind == OMPC_lastprivate || Kind == OMPC_linear ||
      Kind == OMPC_reduction; // TODO add next clauses like 'reduction'.

  if (Kind != OMPC_unknown && isOpenMPPrivate)
    return true;

  DeclRefExpr *DE;
  return DSAStack->hasDSA(VD, Kind, DKind, DE);
}
...
OMPClause *Sema::ActOnOpenMPSingleExprClause(OpenMPClauseKind Kind, Expr *Expr,
  ...
  case OMPC_affinity:
    ...
    break;
  case OMPC_priority:
    Res = ActOnOpenMPPriorityClause(Expr, StartLoc, EndLoc);
    break;
  case OMPC_collapse:
...
OMPClause *Sema::ActOnOpenMPPriorityClause(Expr *Priority,
  ...
  if (!ValExpr->isTypeDependent() && !ValExpr->isValueDependent() &&
      !ValExpr->isInstantiationDependent()) {
    SourceLocation Loc = Priority->getExprLoc();
    ExprResult Value =
        PerformContextualImplicitConversion(Loc, Priority, ConvertDiagnoser);
    if (Value.isInvalid() ||
        !Value.get()->getType()->isIntegralOrUnscopedEnumerationType())
      return 0;
  ...
  return new (Context) OMPPriorityClause(ValExpr, StartLoc, EndLoc);
}
